...
- The notch has a gallery of visuals, all generated in code: eight your voice drives while you talk (wave, mirror, bars, ripple, fire, comet, rain, pulse) and five that play while it transcribes (scanner, orbit, snake, ember, drizzle), each one-sided or two-sided as it suits. Settings → Voice shows in one line which transcription model is in use. Settings → Voice folds into collapsible cards in a sensible order, its controls show on light themes, the upload model is labelled as the fallback it is, and a voice log entry copies on click.
