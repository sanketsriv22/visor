...
- "Check for Updates…" now shows progress in a small floating pill just below the menu-bar icon (with a spinner: Checking → Downloading → Installing → Restarting), instead of text in the menu — which closed the moment you clicked, hiding all feedback. The pill stays clear of crowded menu bars and clamps on-screen; "You're on the latest" / errors flash there briefly. (Updates already relaunch the app for you automatically; this just makes that visible.)
